...
class GoodGenerator(nn.Module):
    def __init__(self, dim=DIM, output_dim=OUTPUT_DIM, ctrl_dim=0):
        ...
        self.rb4 = ResidualBlock(1*self.dim, 64, 3, resample = 'up')
        self.rb5 = ResidualBlock(64, 32, 3, resample = 'up')
        self.bn  = nn.BatchNorm2d(32)
        self.conv1 = MyConvo2d(input_dim=32, output_dim=1, kernel_size=3)
        self.relu = nn.ReLU()
...
class GoodDiscriminator(nn.Module):
    def __init__(self, dim=DIM):
        ...
        self.rb2 = ResidualBlock(2*self.dim, 3*self.dim, 3, resample = 'down', hw=int(DIM/2))
        self.rb3 = ResidualBlock(3*self.dim, 4*self.dim, 3, resample = 'down', hw=int(DIM/4))
        self.rb4 = ResidualBlock(4*self.dim, 4*self.dim, 3, resample = 'down', hw=int(DIM/8))
        self.ln1 = nn.Linear(8*8*512, 1)
